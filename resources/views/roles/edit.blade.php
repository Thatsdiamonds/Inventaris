...
@section('content')
    ...
    <form action="{{ route('roles.update', $role->id) }}" method="POST"
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="notes" style="width: 100%; padding: 8px; margin-top: 5px; height: 60px;">{{ $role->notes }}</textarea>
        </div>
                    ...
                    @foreach ($availablePermissions as $key => $label)
                        <label style="display: block; margin-bottom: 8px; cursor: pointer;">
                            <input type="checkbox" name="permissions[]" value="{{ $key }}"
                                {{ in_array($key, $role->permissions ?? []) ? 'checked' : '' }}> {{ $label }}
                        </label>
                    @endforeach
                </div>
            </div>
                    ...
                    @foreach ($locations as $loc)
                        <label style="display: block; margin-bottom: 8px; cursor: pointer;">
                            <input type="checkbox" name="location_ids[]" value="{{ $loc->id }}"
                                {{ in_array($loc->id, $selectedLocations) ? 'checked' : '' }}> {{ $loc->name }}
                        </label>
                    @endforeach
                </div>
            </div>
